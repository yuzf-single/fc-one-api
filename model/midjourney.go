...
func UpdateTaskInfo(mj_id string, midjourney *Midjourney) error {
	midjourneyInfo := &Midjourney{
		Code:        midjourney.Code,
		Prompt:      midjourney.Prompt,
		PromptEn:    midjourney.PromptEn,
		Description: "提交成功",
	}
	return DB.Model(&Midjourney{}).Where("mj_id = ?", mj_id).Updates(midjourneyInfo).Error
}
