...
func relayMidjourneySubmit(c *gin.Context, relayMode int) *MidjourneyResponse {
	...
	if relayMode == RelayModeMidjourneyImagine { //绘画任务，此类任务可重复
	...
	} else if relayMode == RelayModeMidjouneryModal { //蒙层任务
		...
		if originTask.Action != "CUSTOMZOOM" && originTask.Action != "INPAINT" {
			return &MidjourneyResponse{
				Code:        4,
				Description: "task_oper_no_support",
			}
		}
		if originTask.Action == "CUSTOMZOOM" {
			//用户自定义zoom
			if !strings.Contains(midjRequest.Prompt, "--zoom") {
				return &MidjourneyResponse{
					Code:        4,
					Description: "new_prompt_not_contain_--zoom",
				}
			}
		}
		if midjRequest.MaskBase64 == "" {
			return &MidjourneyResponse{
				Code:        4,
				Description: "maskBase64_is_empty",
			}
		}
		...
		midjRequest.Action = originTask.Action
		...
		if originTask == nil {
		...
		} else if originTask.Status != "SUCCESS" && originTask.Action != "INPAINT" {
			return &MidjourneyResponse{
				Code:        4,
			...
			if relayMode == RelayModeMidjouneryAction {
				if midjRequest.CustomId == "" {
					return &MidjourneyResponse{
						Code:        4,
						Description: "customId_is_null",
					}
				}
				customIdList := strings.Split(midjRequest.CustomId, "::")
				midjRequest.Action = splitAndGetCustomAction(customIdList)
				if midjRequest.Action == "BOOKMARK" {
					return &MidjourneyResponse{
						Code:        4,
						Description: "customId NOT SUPPORT BOOKMARK",
					}
				}
			}
			c.Set("base_url", channel.GetBaseURL())
			c.Set("channel_id", originTask.ChannelId)
			log.Printf("检测到此操作为: %s，获取原channel信息: %s,%s", midjRequest.Action, strconv.Itoa(originTask.ChannelId), channel.GetBaseURL())
		}
		midjRequest.Prompt = originTask.Prompt
	}
	log.Printf("midjRequest: %v", midjRequest)
	// map model name
	...
	if relayMode == RelayModeMidjouneryAction {
		if midjRequest.Action != "PAN" && midjRequest.Action != "REROLL" {
			sizeRatio = 0.2
		}
	}
	...
	if relayMode == RelayModeMidjouneryModal {
		if midjRequest.Action == "CUSTOMZOOM" || midjRequest.Action == "INPAINT" {
			err := model.UpdateTaskInfo(midjourneyTask.MjId, midjourneyTask)
			if err != nil {
				return &MidjourneyResponse{
					Code:        4,
					Description: "update_midjourney_task_failed",
...
func splitAndGetCustomAction(customIdList []string) string {
	step1Action := customIdList[1]
	step2Action := customIdList[2]
	if step1Action == "JOB" {
		action := strings.ReplaceAll(strings.ToUpper(step2Action), "UPSAMPLE", "UPSCALE")
		if strings.Contains(action, "UPSCALE") {
			return "UPSCALE"
		}
		if strings.Contains(action, "VARIATION") {
			return "VARIATION"
		}
		if strings.Contains(action, "PAN") {
			return "PAN"
		}
		return action
	}
	return strings.ReplaceAll(strings.ToUpper(step1Action), "UPSAMPLE", "UPSCALE")
}
